...
def _refineTS(pot, coords, tsSearchParams=dict(), eigenvec0=None, pushoff_params=dict()):
    ...
    coords = ret.coords
    if not ret.success:
        logging.info("transition state search failed")
        return False, ret, None, None
        
    if ret.eigenval >= 0.:
        logging.info("transition state has positive lowest eigenvalue, skipping: %s %s %s", ret.eigenval, ret.energy, ret.rms)
        logging.info( "         not adding transition state")
        return False, ret, None, None
    
    #find the minima which this transition state connects
    logging.info("falling off either side of transition state to find new minima")
...
class LocalConnect(object):
    ...
    def _refineTransitionStates(self, neb, climbing_images):
        ...
        for energy, i in climbing_images[:nrefine]:
            count += 1
            logging.info( "")
            logging.info( "refining transition state from NEB climbing image: %s %s %s", count, "out of", nrefine)
            coords = neb.coords[i,:]
    ...
    def _doNEB(self, minNEB1, minNEB2, repetition = 0):
        ...
        if repetition == 0: 
            ...
            logging.info("running NEB a second time")
            logging.info("    doubling the number of images")
            logging.info("    doubling the number of steps")
            factor = float(repetition + 1)
        
        logging.info( "starting NEB run to try to connect minima %s %s %s", minNEB1._id, minNEB2._id, dist)
        ...
        if self.reoptimize_climbing > 0:
            logging.info( "optimizing climbing images for a small number of steps")
...
#            neb.optimize(**NEBquenchParams)
    ...
    def connect(self, min1, min2):
        ...
        for repetition in range(self.NEBattempts):
            ...
            logging.info( "from NEB search found %s %s", nclimbing, "transition state candidates")
            if nclimbing > 0:
