...
class DoubleEndedConnect(object):
    ...
    def __init__(self, min1, min2, pot, mindist, database, 
        ...
        if self.graph.areConnected(self.minstart, self.minend):
            logging.info("minima are already connected.  not initializing distance graph")
            return
        ...
        if self.verbosity > 0:
            logging.info("************************************************************")
            logging.info("starting a double ended connect run between")
            logging.info("        minimum 1: id %d energy %f" % (self.minstart._id, self.minstart.energy))
            logging.info("        minimum 2: id %d energy %f" % (self.minend._id, self.minend.energy))
            logging.info("        dist %f" % self.getDist(self.minstart, self.minend))
            logging.info("************************************************************")
    ...
    def mergeMinima(self, min1, min2):
        ...
        logging.info( "merging minima %s %s %s %s %s", min1._id, min2._id, dist, "E1-E2", min1.energy - min2.energy)
        ...
        if ((min1 == self.minstart and min2 == self.minend) or 
            ...
            logging.error( "ERROR: trying to merge the start and end minima.  aborting")
            return
        ...
        if dist > self.max_dist_merge:
            logging.info( "    minima merge aborted.  distance is too large %s", dist)
            return
...
#                    print "error, a transition state attached to min2 is still in database", ts.minimum1._id, ts.minimum2._id
        ...
        if debug:
            ...
            if min2 in self.graph.graph.nodes():
                logging.error( "error, min2 is still in self.graph.graph")
            logging.debug( "self.graph.graph. nnodes %s", self.graph.graph.number_of_nodes() )
    ...
    def _addTransitionState(self, E, coords, min_ret1, min_ret2, eigenvec, eigenval):
        ...
        if E < me1 or E < me2:
            logging.warning("trying to add a transition state that has energy lower than its minima.")
            logging.warning("    TS energy %s %s %s %s", E, "minima energy", me1, me2 )
        ...
        if min1 == min2:
            logging.warning( "stepping off the transition state resulted in twice the same minima %s", min1._id)
            return False
        
        

        logging.info("adding transition state %s %s", min1._id, min2._id)
...
#        self.graph.refresh()
        ...
        if self.verbosity > 1:
            ...
            if min1 != self.minstart and min1 != self.minend:
                ...
                logging.info("    distances: %4d -> %4d = %f    %4d -> %4d = %f    %4d -> %4d = %f  %s" % (msid, m1id, ds, m1id, meid, de, m1id, m2id, dse, triangle))
            ...
            if min2 != self.minstart and min2 != self.minend:
                ...
                if ds < dse > de:
                    ...
                    triangle = ": new minima not in between start and end"
                logging.info( "    distances: %4d -> %4d = %f    %4d -> %4d = %f    %4d -> %4d = %f" % (msid, m2id, ds, m2id, meid, de, m2id, m2id, dse))
    ...
    def _localConnect(self, min1, min2):
        ...
        if self.pairsNEB.has_key((min1, min2)):
            logging.warning("WARNING: redoing NEB for minima %s %s", min1._id, min2._id)
            logging.warning("         aborting NEB")
        ...
        if self.graph.areConnected(min1, min2):
            logging.warning("in _local_connect, but minima are already connected. aborting %s %s %s", min1._id, min2._id, self.getDist(min1, min2))
            self.dist_graph.setTransitionStateConnection(min1, min2)
        ...
        if nsuccess == 0:
            dist = self.getDist(min1, min2)
            if dist < self.max_dist_merge:
                logging.warning("local connect failed and the minima are close. Are the minima really the same?")
                logging.warning("         energies: %s %s %s %s", min1.energy, min2.energy, "distance", dist) 
                if self.merge_minima:
                    self.mergeMinima(min1, min2)
                else:
                    logging.warning("         set merge_minima=True to merge the minima") 
                return False   
    ...
    def _getNextPair(self):
        ...
        """
        logging.info("finding a good pair to try to connect")
        #get the shortest path on dist_graph between minstart and minend
        if True:
            logging.debug("Gdist has %s %s %s %s", self.dist_graph.Gdist.number_of_nodes(), 
        ...
        if path is None or weightsum >= 10e9:
            logging.warning("Can't find any way to try to connect the minima")
            return None, None
...
#                w = weights.get((min2,min1))
        ...
        if True:
            #print the path
            logging.info("best guess for path.  (dist=0.0 means the path is known)")
            for w, min1, min2 in weightlist:
                if w > 1e-6:
                    dist = self.getDist(min1, min2)
                else:
                    dist = w
                logging.info("    path guess %s %s %s", min1._id, min2._id, dist)
    ...
    def connect(self):
        ...
        for i in range(self.niter):
            ...
            if self.graph.areConnected(self.minstart, self.minend):
                ...
                logging.info("found connection!")
                return
            
            logging.info("")
            logging.info("======== starting connect cycle %s %s", i, "========")
        ...
        logging.info("failed to find connection between %s %s", self.minstart._id, self.minend._id)
