...
cdef extern from "pele/pspin_spherical.h" namespace "pele":
    cdef cppclass cMeanFieldPSpinSpherical "pele::MeanFieldPSpinSpherical"[p]:
        cMeanFieldPSpinSpherical(_pele.Array[double] interaction, size_t nspins, double tol, cbool spin_zero_frozen) except +
...
cdef class _Cdef_MeanFieldPSpinSpherical(_pele.BasePotential):
    ...
    cdef int p
    cdef double tol
    cdef cbool spin_zero_frozen
    
    def __cinit__(self, np.ndarray[double, ndim=1] interactions, int nspins, int p, double tol=1e-7, cbool spin_zero_frozen=False):
        
        self.nspins = nspins
        self.p = p
        self.interactions = interactions
        self.tol = tol
        self.spin_zero_frozen = spin_zero_frozen
        ...
        if self.p == 2:
            self.thisptr = shared_ptr[_pele.cBasePotential]( <_pele.cBasePotential*>new cMeanFieldPSpinSpherical[INT2](array_wrap_np(self.interactions), 
                                                                                                                 self.nspins, self.tol, self.spin_zero_frozen) )
        elif self.p == 3:
    ...
    def __reduce__(self):
        return (MeanFieldPSpinSpherical,(self.interactions, self.nspins, self.p, self.tol, self.spin_zero_frozen))
