#ifndef PYGMIN_PAIRWISE_POTENTIAL_H
#define PYGMIN_PAIRWISE_POTENTIAL_H
...
{
    /**
     * Define a base class for potentials with pairwise interactions that
    ...
    class PairwisePotential : public BasePotential
    {
    protected:
        pairwise_interaction *_interaction;
        distance_policy *_dist;

        PairwisePotential(pairwise_interaction *interaction,
    ...
    inline double PairwisePotential<pairwise_interaction,distance_policy>::get_energy_gradient(Array<double> x, Array<double> grad)
    {
        double e=0.;
        double gij, dr[3];
        ...
        for(size_t i=0; i<natoms; ++i) {
            ...
            for(size_t j=i+1; j<natoms; ++j) {
                ...
                e += _interaction->energy_gradient(r2, &gij, i ,j);
                for(size_t k=0; k<3; ++k)
                    grad[i1+k] -= gij * dr[k];
                for(size_t k=0; k<3; ++k)
                    grad[i2+k] += gij * dr[k];
    ...
    template<typename pairwise_interaction, typename distance_policy>
    inline double PairwisePotential<pairwise_interaction, distance_policy>::get_energy(Array<double> x)
    {
        double e=0.;
