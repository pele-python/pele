...
class MetropolisTest(unittest.TestCase):
    ...
    def test_heat_capacity(self):
        ...
        self.Emax = 5 #this choice is fundamentally arbitrary, it's only used to generate the initial configuration
        ...
        for T in temperatures:
            print T
            mcrunner = Metropolis_MCrunner(self.potential, self.start_coords,  temperature=T, niter=1e6, stepsize=1, 
                                       adjustf = 0.9, adjustf_niter = 10000, radius=1000000)
            #MCMC 
            mcrunner.run()
            #collect the results
            ...
            self.assertLess(cv-self.ndim,0.1)
